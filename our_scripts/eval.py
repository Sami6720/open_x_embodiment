...
from real_robot import RealRobotics
...
def get_episodes(target_episode_count, target_instruction):
    source_dir_path = os.path.join(COLLECTED_EPISODES_SOURCE_DIR_PATH, '_'.join(target_instruction.split(' ')), str(target_episode_count))
    ...
    for episode_num in range(target_episode_count):
        episodes.append(tf.data.experimental.load(os.path.join(source_dir_path, str(episode_num))))
    return episodes

def create_and_save_fig(rbg_array, instruction, file_ending):
    frame_img_save_path = os.path.join(FRAMES_DESTINATION_DIR_PATH, f"{file_ending}.png")
    ...
    with open(os.path.join(RESULTS_DIR, f"result_{random_file_ending}.txt"), 'w') as f:
        ...
        for index, episode in enumerate(episodes):
            ...
            for step in episode:
                ...
                frame_img_path = create_and_save_fig(observation['rgb'], observation['instruction'], random_file_ending)
...
if __name__=='__main__':
   ...
   parser.add_argument('--target_episode_count', default=4, help="Number of episodes wanted.")
   parser.add_argument('--target_instruction', default='push the red star to the bottom right of the board')
