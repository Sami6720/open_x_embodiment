...
def collect_episodes(target_episode_count, target_instruction):
  episodes = []
  try:
    episode_count = 0
    ...
    for episode in get_episode_ds(dataset_paths['language_table']):
        ...
        if instruction.strip() == target_instruction.strip():
            ...
            print(f"Current episode count with target instruction: {len(episodes)}")
        if len(episodes) == target_episode_count:
          print(f"{target_episode_count} episodes have been reached.")
...
if __name__=='__main__':
   ...
   collect_episodes(args.target_episode_count, args.target_instruction)
